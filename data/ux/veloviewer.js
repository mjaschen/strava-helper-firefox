var StravaHelper = (function(sh) {

    var logger = new sh.Logger('ux.veloviewer');

    sh.ux.veloviewer = {
        addVeloViewerLinks: function() {

            if (sh.prefs.addVeloViewerLinks) {
                addVeloViewerLink();
                addVeloViewerActivityLink();
            }
        }
    };

    // Adds a link to veloviewer.com in the top menu
    function addVeloViewerLink() {
        var $link = $("<a/>", {
            href: "http://veloviewer.com/",
            text: "VeloViewer",
            css: {
                color: '#ff032e'
            }
        });

        var $listItem = $("<li/>", {
            class: "nav-link"
        });

        $listItem.append($link).appendTo("ul.global-nav");
    }

    // Adds a link to the activity on veloviewer.com to the activity page
    function addVeloViewerActivityLink() {
        if (!sh.util.isCurrentPage(['activities'])) {
            return;
        }

        logger.debug('adding link to veloviewer activity page');

        try {
            var activityId = sh.util.getActivityId();
        } catch (e) {
            logger.error(e);
            return;
        }

        var $veloviewer = $("section.others-section div.flybys").clone();

        $veloviewer
            .removeClass("flybys")
            .addClass("veloviewer")
        $veloviewer
            .find("div.labs-logo img")
                .attr("src", "data:img/png;base64,");
        $veloviewer
            .find("a")
                .attr("href", "http://veloviewer.com/activities/" + activityId.toString())
                .text("View Activity");

        $("section.others-section").append($veloviewer);
    }

    return sh;
}(StravaHelper));